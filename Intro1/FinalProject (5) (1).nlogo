...
turtles-own [ MaxStamina Stamina Tired? ]
...
to flaghunt
    ifelse any? flags in-radius radius-of-runners with [ taken? = false and by? = false ] [
      face one-of flags in-radius radius-of-runners with [ taken? = false and by? = false ]
      fd speed-of-runners
      if any? flags-here with [ by? = false and taken? = false ] [
      ask flags-here [
        set by? [ who ] of myself
        set taken? "Defenders" ] ] ]
      [ run-away Attackers ]
...
to flagcheck
  if any? flags with [ taken? = "Attackers" ] [
    set State "Defending" ]
  if any? flags with [ taken? = false or taken? = "Defenders" ] [
    set state "Searching" ]
end
...
to flagsteal
  if any? flags-here with [ taken? = "Defenders" or taken? = false ] [
    ask flags-here [
      set by? [ who ] of myself
      set taken? "Attackers" ]
    set state "Returning" ]
