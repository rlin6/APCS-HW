...
public class Pig
{
  ...
  public static String changeCapital(String c, boolean capital) 
  {
		...
		{ //if the word originally had a capital letter at the beginning
			...
			c = c.substring(1); //takes away the original letter from the old string
  ...
  public static boolean yVowel(String s) 
	{ //checks to see if there's y is the first vowel in the string
		...
		//checks to see if the string has a y,the y is not the first letter(otherwise it'd be a consonant), and that there is no vowel before it(or else it would just end at that vowel) 
		if (hasA(lower, "y") && lower.substring(0, 1) != "y" && allVowels(s.substring(0, yIndex(s))) == "") 
		{
	  		return true; //if all these conditions are met, y would be the first vowel in the string
		}
  ...
  public static String wordToPig(String w) 
  {
		...
		if ((w != null) && w.length() > 0) 
		{
			if ((ALPHABET.toUpperCase()).indexOf(w.substring(0, 1)) != -1) 
			{
				hasCapital = true;
			}
	  	...
	  	//otherwise, if the 1st vowel is y acting like a vowel:
			else if (yVowel(w)) 
			{
				...
				for (int i = 0; i < w.length(); i++) 
				{
					...
					if (w.substring(i, i + 1).equals(firstVowel(w))) 
					{
						cons = w.substring(0, i);
						String post = w.substring(i, w.length()); //String to hold the letters after the begining consonants
						return changeCapital(post + cons + "ay", hasCapital);
					}
      			}
  			}
		}
  	return changeCapital(w, hasCapital);	
	}
