...
to setup
  ca
  reset-ticks
  if image-name = "savitar" [
    import-pcolors "savitar.jpg" ]
  if image-name = "pencils" [
    import-pcolors "pencils.jpg" ]
  if image-name = "pusheen" [
...
to blur
  grayscale
  ask patches [ 
    set next-Color ( ( ( 1 - realpercent ) * pcolor ) + ( realpercent * average ) ) ]
  change
end
